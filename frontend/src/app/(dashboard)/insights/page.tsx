...
import {
  Banner, Button, Card, ChartScroll, EmptyState, GroupedBarChart, Meter,
  PageHeader, PercentLineChart, SegmentedControl, SkeletonCard,
} from "@/components/ui";
...
export default function InsightsPage() {
  ...
  return (
    <div>
      ...
      {data && (
        <>
          {/* Stacked, not side by side: a 12-month series needs ~670px, and
              two columns starve both charts until well past 1440px. */}
          <div className="mb-6 grid gap-4">
            <Card title="Collected vs billed">
              <ChartScroll minWidth={Math.max(320, data.revenue.length * 56)}>
                <GroupedBarChart
              ...
              <p className="mt-2 text-[11px] leading-relaxed text-stone-400">
                Collected is counted in the month the money arrived, so arrears
                cleared in one payment show as a single tall bar.
              </p>
            </Card>

            <Card title="Occupancy">
              <ChartScroll minWidth={Math.max(320, data.occupancy.length * 56)}>
                <PercentLineChart
          ...
          <Card title="By room" padding="none">
            {!hasRooms ? (
              <div className="p-4">
                <EmptyState
                  title="No rooms yet"
